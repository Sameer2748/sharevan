...
export default function BookingPage() {
  ...
  useEffect(() => {
    ...
    // If user is authenticated, redirect to authenticated booking page
    if (isAuthenticated) {
    ...
    // Try to restore saved booking data
    ...
    if (savedBooking) {
      try {
        ...
        setCurrentStep(data.currentStep || 'locations');
  ...
  // Auto-save booking data as user progresses
  useEffect(() => {
  ...
  }, [formData, bookingType, scheduleDetails, currentStep, priceEstimate, hasHydrated, isAuthenticated]);
  ...
  const fetchPriceEstimate = async () => {
    if (!formData.pickupAddress || !formData.deliveryAddress) {
      console.log('Missing addresses:', { pickup: formData.pickupAddress, delivery: formData.deliveryAddress });
      return;
    }

    // Validate coordinates are not 0
    if (formData.pickupLat === 0 || formData.pickupLng === 0 || formData.deliveryLat === 0 || formData.deliveryLng === 0) {
      console.error('Invalid coordinates:', {
        pickup: { lat: formData.pickupLat, lng: formData.pickupLng },
        delivery: { lat: formData.deliveryLat, lng: formData.deliveryLng }
      });
      toast.error('Please select valid locations from the suggestions or map');
      return;
    }
  ...
  const handleBookNow = async () => {
    ...
    try {
      ...
      toast.success('Booking details saved! Please login to complete your order.');

      // Redirect to login with return URL
      router.push('/auth/login?role=user&returnUrl=/user/booking&fromBooking=true');
    } catch (error: any) {
          ...
          <div className="pb-6">
            ...
            <button
              onClick={() => {
                ...
                // Validate coordinates before proceeding to price review
                if (formData.pickupLat === 0 || formData.pickupLng === 0 || formData.deliveryLat === 0 || formData.deliveryLng === 0) {
                  toast.error('Please select valid pickup and delivery locations from the suggestions or map');
                  return;
                }
